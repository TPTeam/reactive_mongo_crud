...
trait FatherPersistenceCompanion[T <: ModelObj, R <: ModelObj] {
  self: RefPersistenceCompanion[T]=>

  val CHILD: PersistenceCompanion[R] with SonPersistenceCompanion[R,T]
  ...
  private[models] def updateDownOnUpdate(id: BSONObjectID, obj: T) = {
    ...
    for {
     ...
     // remove the new granpa sons from the other granpas
      removeFrom(getSons(obj), granpas.map(i => Reference(i))).onComplete{
        _ => removeFromGPBlock.trySuccess(true)
      }
